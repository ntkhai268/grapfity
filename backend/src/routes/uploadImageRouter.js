...
if (!fs.existsSync(playlistImageDir)) {
    fs.mkdirSync(playlistImageDir, { recursive: true });
...
router.post(
    '/playlist-image', // Đường dẫn con sau /api/upload
    authenticateUser, // 1. Xác thực người dùng trước khi cho upload
    (req, res, next) => { // 2. Middleware gọi multer để xử lý file
        upload(req, res, (err) => {
