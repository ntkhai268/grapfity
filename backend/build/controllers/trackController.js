...
const getMyTracksController = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      message: 'Lỗi server khi lấy danh sách bài hát đã tải lên.'
    });
  }
};
...
const getPublicTracksOfUserController = async (req, res) => {
  ...
  try {
    ...
    if (!userId || isNaN(Number(userId))) {
      return res.status(400).json({
        message: 'userId không hợp lệ trong URL.'
      });
    }
    ...
    return res.status(500).json({
      message: 'Lỗi server khi lấy danh sách bài hát công khai.'
    });
  }
};
...
const uploadTrackCoverController = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      error: 'Lỗi server khi upload ảnh track.'
    });
  }
};
...
const updateTrackController = async (req, res) => {
  ...
  if (!id || !userId) {
    return res.status(400).json({
      message: 'Thiếu ID hoặc chưa đăng nhập.'
    });
  }
  try {
    const updateData = {};
    ...
    if (title || lyrics !== undefined) {
      ...
      await db.Metadata.update(metadataUpdate, {
        where: {
          track_id: id
        }
      });
...
const downloadTrackController = async (req, res) => {
  try {
    ...
    if (!track.trackUrl) {
      return res.status(404).json({
        error: 'Không có đường dẫn âm thanh.'
      });
    }
