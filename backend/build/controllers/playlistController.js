...
import { createPlaylist, getAllPlaylistsByUserId, getAllPublicPlaylists, updatePlaylist, deletePlaylist
// getPlaylistById
// addTrackToPlaylist,
  ...
  try {
    ...
    if (!userId) {
      ...
      return res.status(401).json({
        error: 'Unauthorized: Thông tin người dùng không hợp lệ.'
      });
    }
    ...
    res.status(500).json({
      error: 'Lỗi server khi lấy danh sách playlist.'
    });
  }
};
...
const getPublicPlaylistsController = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res.status(400).json({
        error: 'Thiếu userId trong request.'
      });
    }
    ...
    res.status(500).json({
      error: 'Lỗi server khi lấy playlist công khai.'
    });
  }
};
...
const createPlaylistController = async (req, res) => {
  try {
    ...
    const {
      trackId
    } = req.body; // trackId có thể là undefined
    if (!userId) {
    ...
    res.status(500).json({
      error: 'Lỗi server khi tạo playlist.'
    });
  }
};
...
const deletePlaylistController = async (req, res) => {
  try {
    ...
    const {
      playlistId
    } = req.params; // Lấy playlistId từ tham số URL
...
const uploadPlaylistCoverController = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      error: 'Lỗi server khi xử lý upload ảnh.'
    });
  }
};
...
const updatePlaylistController = async (req, res) => {
  try {
    ...
    if (!playlistId || isNaN(Number(playlistId))) {
      return res.status(400).json({
        error: 'Bad Request: ID của playlist không hợp lệ hoặc thiếu.'
      });
    }
    if (typeof title !== 'string' || title.trim() === '') {
      return res.status(400).json({
        error: 'Bad Request: Tiêu đề playlist không được để trống.'
      });
    }
    if (imageUrl !== undefined && imageUrl !== null && typeof imageUrl !== 'string') {
      return res.status(400).json({
        error: 'Bad Request: Định dạng imageUrl không hợp lệ.'
      });
    }
    if (privacy !== undefined && privacy !== 'public' && privacy !== 'private') {
      return res.status(400).json({
        error: 'Bad Request: Giá trị privacy không hợp lệ.'
      });
    }
    const updatedPlaylist = await updatePlaylist(Number(playlistId), userId, title.trim(), imageUrl, privacy);
    return res.status(200).json({
    ...
    if (statusCode === 404) errorMessage = 'Không tìm thấy playlist.';else if (statusCode === 403) errorMessage = 'Bạn không có quyền cập nhật playlist này.';else if (statusCode === 400) errorMessage = err.message || 'Dữ liệu gửi lên không hợp lệ.';
    return res.status(statusCode).json({
      error: errorMessage
    });
  }
...
export { getMyPlaylistsController, getPublicPlaylistsController, getAllPublicPlaylistsController, createPlaylistController, uploadPlaylistCoverController, updatePlaylistController, deletePlaylistController };