...
const getLikedTracksByUserController = async (req, res) => {
  ...
  try {
    ...
    const formattedTracks = likedTrack.filter(like => like.Track).map(like => like.Track);
    res.status(200).json({
...
export { likeTrackController, getLikedTracksByUserController, unlikeTrackController, isTrackLikedByUserController, countLikesForTrackController };