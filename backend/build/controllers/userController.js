...
import { getAllUsers, createUserService, getUserById, handleUserLogin, updateUser, deleteUser } from '../services/user_service.js';
...
const registerController = async (req, res) => {
  try {
    ...
    if (!userName || !email || !password || roleId === undefined) {
      return res.status(400).json({
        message: 'userName, email, password và roleId là bắt buộc'
      });
    }
...
const verifyPasswordController = async (req, res) => {
  try {
    ...
    if (!password) {
      return res.status(400).json({
        valid: false,
        message: 'Thiếu mật khẩu cần xác minh.'
      });
    }
...
export { getAllUsersController, getUserProfileController, registerController, handleUserLoginController, updateUserController, deleteUserController, logoutController, verifyPasswordController };