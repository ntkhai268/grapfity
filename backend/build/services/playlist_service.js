  ...
  try {
    ...
    const playlists = await db.Playlist.findAll({
      where: {
        ...
        ...(isOwner ? {} : {
          privacy: 'public'
        }) // 👈 Nếu không phải chủ sở hữu, chỉ thấy public
      },
      include: [{
        model: db.User,
...
const deletePlaylist = async (playlistId, userId) => {
  ...
  try {
    ...
    const playlistToDelete = await db.Playlist.findOne({
      where: {
        id: numericPlaylistId,
        // Giả sử cột ID trong Playlist là 'id'
        // --- SỬA TÊN CỘT Ở ĐÂY ---
    ...
    if (t && !t.finished) {
      // Kiểm tra xem transaction đã kết thúc chưa
      await t.rollback();
...
export { getAllPlaylistsByUserId, getAllPublicPlaylists, createPlaylist, updatePlaylist,
// getPlaylistById,
deletePlaylist };