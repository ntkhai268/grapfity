...
const trackingListeningHistory = async (userId, trackId) => {
  const [history, created] = await db.listeningHistory.findOrCreate({
    ...
    defaults: {
      listenCount: 1
    } // Nếu mới, bắt đầu từ 1
  });
  if (!created) {
    await history.increment('listenCount', {
      by: 1
    });
  }
  return history;
...
const getTop10PopularTracks = async () => {
  ...
  const results = await listeningHistory.findAll({
    attributes: ['trackId', [db.Sequelize.fn('SUM', db.Sequelize.col('listenCount')), 'totalListens'], [db.Sequelize.fn('MAX', db.Sequelize.col('listeningHistory.updatedAt')), 'lastHeardAt']],
    include: [{
      model: db.Track,
      where: {
        privacy: 'public'
    ...
    group: ['trackId', 'Track.id', 'Track.trackUrl', 'Track.imageUrl', 'Track.uploaderId', 'Track->Metadatum.track_id', 'Track->Metadatum.trackname', 'Track->User.id', 'Track->User.Name'],
    order: [[db.Sequelize.literal('totalListens'), 'DESC'], [db.Sequelize.literal('lastHeardAt'), 'DESC']]
  });
...
export { getListeningHistoryOfUser, trackingListeningHistory, getTop10PopularTracks, getTop5TracksOfUser, getTop5TracksByOwner };